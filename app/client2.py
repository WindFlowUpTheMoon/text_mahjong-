...
class Client:
    ...
    def print_playercards(self, pgcards, handcards):
        ...
        lp1 = [i[0] for i in pgcards['viewable'] + pgcards['unviewable']]
    ...
    def check_card_remainnum(self, card):
        ...
        if card[1] in self.maj.types:
            num += Counter(self.hand_cards[card[1]])[card]
        else:
            num += Counter(self.hand_cards['字'])[card]
        for id, pgcards, *_ in self.otherplayers_cards:
            num += Counter(pgcards['viewable'])[card_num]
        print(card + '剩余：' + str(c[card] - self.leftcards2[card_num] - num))
    ...
    def handle_getcard(self, msg):
        ...
        print('摸到牌：', self.maj.cards_map[card])
